...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        # Broadcast to group (timestamp in UTC Z format)
        try:
            ts = msg.timestamp
            if ts is not None:
                if ts.tzinfo is None:
                    ts = ts.replace(tzinfo=_dt.timezone.utc)
                ts_utc = ts.astimezone(_dt.timezone.utc).isoformat().replace('+00:00', 'Z')
            else:
                ts_utc = None
        except Exception:
            ts_utc = _as_utc_z(msg.timestamp) if msg.timestamp else None
        ...
        try:
            ...
            await self.channel_layer.group_send(
                self.group_name,
                {
                    ...
                    "content": content,
                        "timestamp": (msg.timestamp.astimezone(_dt.timezone.utc).isoformat().replace('+00:00', 'Z') if msg.timestamp is not None else None)
                }
            )
